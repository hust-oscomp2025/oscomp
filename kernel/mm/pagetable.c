...
pagetable_t g_kernel_pagetable;
 ...
 pte_t* pagetable_walk(pagetable_t pagetable, uaddr va, int alloc) {
		 ...
		 for (int level = 0; level < PAGE_LEVELS - 1; level++) {
				 // 计算当前级别的页表索引
				 int index = (va >> (PAGE_SHIFT + PT_INDEX_BITS * (PAGE_LEVELS - 1 - level))) & PT_INDEX_MASK;
 ...
 /**
	* 在页表中映射虚拟地址到物理地址
	*/
 int pagetable_map(pagetable_t pagetable, uaddr va, uint64 pa, uint64 size, int perm) {
		 if (pagetable == NULL) {
				 return -1;
		 }
		 ...
		 // 逐页映射
		 for (uaddr va_page = start_va, pa_page = start_pa; 
					va_page < end_va; 
					va_page += PAGE_SIZE, pa_page += PAGE_SIZE) {
					
				 // 查找页表项，必要时分配页表
				 pte_t* pte = pagetable_walk(pagetable, va_page, 1);
				 if (pte == NULL) {
						 // 分配页表失败，回滚已映射的页
						 pagetable_unmap(pagetable, start_va, va_page - start_va, 0);
 ...
 /**
	* 解除页表中一块虚拟地址区域的映射
	*/
 int pagetable_unmap(pagetable_t pagetable, uaddr va, uint64 size, int free_phys) {
		 if (pagetable == NULL) {
				 return -1;
		 }
		 ...
		 for (uaddr va_page = start_va; va_page < end_va; va_page += PAGE_SIZE) {
				 // 查找页表项，不分配新页表
				 pte_t* pte = pagetable_walk(pagetable, va_page, 0);
				 if (pte == NULL) {
						 // 页表不存在，跳过
						 continue;
				 }
 ...
 uint64 pagetable_translate(pagetable_t pagetable, uaddr va) {
		 // 查找页表项
		 pte_t* pte = pagetable_walk(pagetable, va, 0);
 ...
 pagetable_t pagetable_copy(pagetable_t src, uaddr start, uaddr end, int share) {
		 ...
		 for (uaddr va = start; va < end; va += PAGE_SIZE) {
				 // 查找源页表项
				 pte_t* src_pte = pagetable_walk(src, va, 0);
				 ...
				 if (share == 0) {
						 ...
						 // 在新页表中创建映射
						 pte_t* dst_pte = pagetable_walk(dst, va, 1);
						 if (dst_pte == NULL) {
								 free_page(new_page);
								 spinlock_unlock_irqrestore(&pagetable_lock, flags);
								 pagetable_free(dst);
 ...
 /**
	...
	*/
 static void _pagetable_dump_level(pagetable_t pagetable, int level, uaddr va_prefix) {
		 // 打印缩进
		 ...
		 for (int i = 0; i < PT_ENTRIES; i++) {
				 ...
				 if (pte & PTE_V) {
						 // 计算此项对应的虚拟地址
						 uaddr va = va_prefix | ((uaddr)i << (PAGE_SHIFT + PT_INDEX_BITS * (PAGE_LEVELS - 1 - level)));
						 ...
						 // 打印索引和虚拟地址
						 char entry_buf[100];
						 sprint(entry_buf, "[%d] VA 0x%lx -> PA 0x%lx, flags: ", 
										i, va, PTE2PA(pte));
						 putstring(entry_buf);
						 
						 // 打印权限标志
						 if (pte & PTE_R) putstring("R");
						 if (pte & PTE_W) putstring("W");
						 if (pte & PTE_X) putstring("X");
						 if (pte & PTE_U) putstring("U");
						 if (pte & PTE_G) putstring("G");
						 if (pte & PTE_A) putstring("A");
						 if (pte & PTE_D) putstring("D");
						 putstring("\n");
