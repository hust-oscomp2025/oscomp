...
inline void ext4_lock(void)
{
	spinlock_lock(&ext4_spinlock);
}

inline void ext4_unlock(void)
{
	spinlock_unlock(&ext4_spinlock);
}
...
static void ext4_lock_noop(void) {
...
void make_fsid_from_uuid(const uint8_t uuid[16], fsid_t *fsid) {
    ...
    fsid->__val[0] = (int)(u[0] ^ u[1]);
    fsid->__val[1] = (int)(u[2] ^ u[3]);
}